...
const ShowRoutes = () => {
			...
			const parsedRoutes = Object.fromEntries(
				Object.entries(routesData).map(([key, route]) => [
					...
					route.map((point: Route) => ({
						lat: Number.parseFloat(point.lat.toString()),
						lng: Number.parseFloat(point.lng.toString()),
					})),
				]),
			);

			const parsedStorage = {
				lat: Number.parseFloat(response.data.storage.lat.toString()),
				lng: Number.parseFloat(response.data.storage.lng.toString()),
			};
			setStoragePosition(parsedStorage);
	...
	const handleDeleteOperation = async (operationId: number) => {
		try {
			await ApiInstance.delete("/operations", {
			...
			<Dialog open={dialogOpen} onOpenChange={() => setDialogOpen(false)}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Estado actual de la ruta {selectedOperationId}
						</DialogDescription>
					</DialogHeader>
					<GoogleMap
